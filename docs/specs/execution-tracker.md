...
| P2-15A | Not started | Confirm every tenant-owned data model has an approved tenant/company key. | Read-only model and migration audit only. Identify all tenant-owned entities, global product-owned entities, join tables, audit tables, upload metadata tables, and legacy tables that lack `CompanyId`, `TenantId`, or an approved tenant key. | Report exact model, migration, and table gaps with file/line references. Do not edit source or database. | Global product configuration may be company-independent only when it contains no client data. Product-owned schematic library assets may be global; schematic assignments are tenant-owned. |
